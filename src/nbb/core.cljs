...
(defn find-file-on-classpath [munged]
  (if-let [roots (cljk-roots)]
    ...
    (let [file (str/replace (str munged) #"\." "/")
        ...
        ;; are probed FIRST so a renamed tree wins over a stale sibling, and in
        ;; the same platform order upstream nbb uses for the old extensions.
        files [(str file ".cljk")
               (str file ".cljs.cljk") (str file ".cljc.cljk")
               (str file ".cljs") (str file ".cljc")
               (str file ".clj.cljk") (str file ".clj")]
