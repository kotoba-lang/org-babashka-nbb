;; OVERLAY of babashka/sci src/sci/impl/analyzer.cljc at the sha deps.edn pins
;; (6790624776b09297a22100438f05b6555a4ff56d). This repo's :paths ["src"] come
;; before the sci git dep on the classpath, so shadow-cljs compiles THIS file.
;; The one change against upstream: return-or / return-and use their
;; already-analyzed children instead of calling `analyze` on them a second time.
;; On CLJS constants are not wrapped in ConstantNode, so the second pass
;; re-resolved the symbols inside quoted constants: `(or nil 'x)` threw
;; "Unable to resolve symbol: x", and kotoba-lang/langchain's langchain.db/q
;; (`(or in '[$])`) made every langchain / langgraph dependent red under kbb.
;; Upstream babashka/sci master (387d116, 2026-09-18) and nbb 1.6.213 still
;; have it. Regression: test-scripts/or_and_quoted_constant_test.cljs.
;; Remove this file when the pinned sci contains the fix (diff it against the
;; pinned upstream file first: any other difference is a bug in this overlay).
  ...
  (let [child-count# (count children)]
    (if (> child-count# 5)
      ...
      (let [children (analyze-children-tail ctx children)
          ...
          0 nil
          1 (nth children 0)
          2 (let [a0# (nth children 0)
                  a1# (nth children 1)]
              (sci.impl.types/->Node
               (or (t/eval a0# ctx bindings)
                   (t/eval a1# ctx bindings))
               nil))
          3 (let [a0# (nth children 0)
                  a1# (nth children 1)
                  a2# (nth children 2)]
              (sci.impl.types/->Node
               (or (t/eval a0# ctx bindings)
